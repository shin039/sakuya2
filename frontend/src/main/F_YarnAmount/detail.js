...
export default function BasicModal(props) {
  ...
  const {d_skuId}       = props.fromParent;
  ...
  useEffect(() => {
    ...
    apiGet({url: `goods/${d_skuId}`, f_success: f_success_sku, commonFunc});

    const f_success_material = response => setMaterialInfo((response && response.data && response.data.result) || {});
    apiGet({url: `material/goods/${d_skuId}`, f_success: f_success_material, commonFunc});
